...
let daftar = JSON.parse(fs.readFileSync('list.json', 'utf8'));
...
if (args2) {
    switch (args) {
        case 'add':
            ...
            let tambah = JSON.stringify(daftar);
            fs.writeFileSync('list.json', tambah);
            break;
        ...
        case 'delete':
            ...
            let kurang = JSON.stringify(daftar);
            fs.writeFileSync('list.json', kurang);
            break;

        case 'complete':
            console.log(`${daftar[index - 1].todo} telah selesai`);
            daftar[index - 1].complete = true;
            let beres = JSON.stringify(daftar);
            fs.writeFileSync('list.json', beres);
            break;
            
        case 'uncomplete':
            console.log(`${daftar[index - 1].todo} status selesai dibatalkan.`);
            daftar[index - 1].complete = false;
            let belum = JSON.stringify(daftar);
            fs.writeFileSync('list.json', belum);
            break;
        ...
        case 'tag':
            ...
            let mark = JSON.stringify(daftar);
            fs.writeFileSync('list.json', mark);
            break;
